...
async function main() {
  const { data: ahmadList, error: ahmadListErr } = await supabase.from('coaches').select('id').eq('name', 'Ahmad');
  let ahmadId;
  ...
  if (ahmadList && ahmadList.length > 0) {
    ...
    if (ahmadErr) { 
      fs.writeFileSync('ahmad_err.json', JSON.stringify(ahmadErr, null, 2));
      throw new Error('Create Ahmad failed, see ahmad_err.json'); 
    }
    ahmadId = newAhmad.id;
  ...
  console.log('Fake data setup complete! The coach name is Ahmad. Check login logic.');
}
