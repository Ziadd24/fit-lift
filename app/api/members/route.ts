...
export async function GET(req: NextRequest) {
  ...
  if (search) {
    ...
    if (safeSearch) query = query.or(`name.ilike.%${safeSearch}%,membership_code.ilike.%${safeSearch}%,email.ilike.%${safeSearch}%`);
  }
  ...
  return NextResponse.json({ members: data, total: count ?? 0, page, pageSize: PAGE_SIZE, totalPages: Math.ceil((count ?? 0) / PAGE_SIZE) });
}
...
export async function POST(req: NextRequest) {
  ...
  if (!name || !membership_code || !sub_expiry_date) return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
  if (name.length > 200) return NextResponse.json({ error: "Name too long" }, { status: 400 });
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase.from("members").insert({
    name: name.slice(0, 200), membership_code: membership_code.slice(0, 50),
    email: (email || "").slice(0, 200) || null, phone: (phone || "").slice(0, 50) || null,
    membership_type: (membership_type || "Basic").slice(0, 100), sub_expiry_date,
  }).select().single();
  if (error) {
    if (error.code === "23505") return NextResponse.json({ error: "Membership code already exists" }, { status: 409 });
