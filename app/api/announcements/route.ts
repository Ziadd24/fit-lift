...
export async function GET(req: NextRequest) {
  ...
  const announcements = (data || []).map((a: any) => ({
    ...
    target_member_id: a.target_member_id, is_global: a.is_global,
    created_at: a.created_at, target_member_name: a.members?.name ?? null,
  }));
...
export async function POST(req: NextRequest) {
  ...
  const sanitizedTitle = sanitizeHtml(title).slice(0, 200);
  const sanitizedContent = sanitizeHtml(content).slice(0, 10000);
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase.from("announcements").insert({
    title: sanitizedTitle, content: sanitizedContent,
    is_global: is_global ?? true, target_member_id: target_member_id || null,
  }).select("*, members(name)").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({
    id: data.id, title: data.title, content: data.content,
    target_member_id: data.target_member_id, is_global: data.is_global,
    created_at: data.created_at, target_member_name: (data as any).members?.name ?? null,
  }, { status: 201 });
}
