...
export default function CaloriesPage() {
  ...
  const { data: rawLogs } = useListCalorieLogs();
  const log = (rawLogs || []).filter(l => l.member_id === null);
  // Client logs (all members today)
  const allClientLogs = (rawLogs || []).filter((l) => l.member_id !== null);
      ...
      <div style={{ fontFamily: "Inter, sans-serif" }}>
        ...
        {activeTab === "clients" && (
          <div className="flex flex-col gap-6">
            ...
            {/* ── 3-column layout: Client List | Food Log | Assessment ── */}
            <div className="flex flex-col xl:flex-row gap-4">
                
              {/* ── Column 1: Client List ── */}
              <div style={{ background: "rgba(22,22,26,0.9)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, padding: 20 }} className="w-full xl:w-56 flex-shrink-0 flex flex-col gap-2">
                <h3 className="font-bold text-white text-xs uppercase tracking-widest mb-2 flex items-center gap-2">
                  <Users className="w-4 h-4" style={{ color: "#7CFC00" }} /> Select Client
                </h3>
                  ...
                  {members.map(m => {
                    ...
                    return (
                      <button key={m.id} onClick={() => setSelectedClientId(m.id)}
                        ...
                        style={{ background: isSelected ? "rgba(124,252,0,0.12)" : "rgba(255,255,255,0.03)", border: isSelected ? "1px solid #7CFC00" : "1px solid rgba(255,255,255,0.05)" }}>
                        <span className="font-bold text-sm" style={{ color: isSelected ? "#7CFC00" : "#fff" }}>{m.name}</span>
                        {mLogs.length > 0 && (
                          <span className="text-black text-[10px] font-black px-2 py-0.5 rounded-full" style={{ background: "#7CFC00" }}>{mLogs.length}</span>
                        )}
                      </button>
                    );
                  })}
                  {members.length === 0 && <span className="text-sm" style={{ color: "#5A5A5A" }}>No clients yet.</span>}
                </div>
              </div>

              {/* ── Column 2: TODAY'S FOOD LOG (main/center) ── */}
              <div className="flex-1 flex flex-col" style={{ background: "rgba(22,22,26,0.9)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, overflow: "hidden" }}>
                {/* Header */}
                <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="font-bold text-white uppercase tracking-wide text-sm">
                      {selectedClientId ? `${selectedMember?.name}'s Food Log` : "Today's Food Log"}
                    </h2>
                    <span className="font-bold text-xs px-2 py-0.5 rounded-full text-black" style={{ background: "#7CFC00" }}>{selectedClientLogs.length}</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-sm" style={{ color: "#8B8B8B" }}>
                    <Flame className="w-4 h-4" style={{ color: "#7CFC00" }} />
                    <span className="font-bold text-white">{selectedClientLogs.reduce((acc, l) => acc + (l.result?.totals?.calories || 0), 0).toLocaleString()}</span> kcal
                  </div>
                </div>
                {/* Progress Summary (Rings) */}
                {selectedClientId && selectedClientLogs.length > 0 && (
                  <div className="px-5 py-6 flex justify-around items-center gap-4 bg-black/20 border-b border-white/5">
                    <MacroRing label="Calories" value={selectedClientLogs.reduce((acc, l) => acc + (l.result?.totals?.calories || 0), 0)} goal={getClientGoals(selectedClientId).calories} unit="kcal" color="#7CFC00" icon={Flame} />
                    <MacroRing label="Protein"  value={selectedClientLogs.reduce((acc, l) => acc + (l.result?.totals?.protein || 0), 0)}  goal={getClientGoals(selectedClientId).protein}  unit="g"    color="#8B5CF6" icon={Zap} />
                    <MacroRing label="Carbs"    value={selectedClientLogs.reduce((acc, l) => acc + (l.result?.totals?.carbs || 0), 0)}    goal={getClientGoals(selectedClientId).carbs}    unit="g"    color="#F59E0B" icon={Wheat} />
                    <MacroRing label="Fat"      value={selectedClientLogs.reduce((acc, l) => acc + (l.result?.totals?.fat || 0), 0)}      goal={getClientGoals(selectedClientId).fat}      unit="g"    color="#10B981" icon={Droplets} />
                  </div>
                )}

                {/* Cards */}
                {selectedClientLogs.length === 0 ? (
                  <div className="flex flex-col items-center justify-center flex-1 py-12 opacity-40">
                    <UtensilsCrossed className="w-10 h-10 mb-3 text-white" />
                    <p className="text-white font-bold text-sm">{selectedClientId ? "No meals logged today" : "Select a client"}</p>
                  </div>
                ) : (
                  <div className="p-4 overflow-y-auto flex flex-col gap-3" style={{ maxHeight: 500 }}>
                    <AnimatePresence>
                      {selectedClientLogs.map((entry) => {
                        const cat = categoryStyle[entry.category as keyof typeof categoryStyle] || { label: "Snack ⚡", bg: "rgba(124,252,0,0.1)", color: "#7CFC00" };
                        return (
                          <motion.div key={entry.id}
                            ...
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-[10px] font-bold px-2.5 py-1 rounded-full" style={{ background: cat.bg, color: cat.color }}>{cat.label}</span>
                              <div className="flex items-center gap-1.5" style={{ color: "#5A5A5A" }}>
                                <Clock className="w-3 h-3" />
                                <span className="text-[10px]">{new Date(entry.created_at || Date.now()).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                              </div>
                            </div>
                            <p className="text-sm font-semibold text-white mb-2 line-clamp-1">{entry.result?.display_title || entry.meal}</p>
                            <div className="flex flex-wrap gap-1.5 mb-2">
                              <span className="text-[10px] font-bold px-2 py-0.5 rounded-lg" style={{ background: "rgba(124,252,0,0.12)", color: "#7CFC00" }}>{entry.result?.totals?.calories || 0} kcal</span>
                              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-lg" style={{ background: "rgba(139,92,246,0.12)", color: "#8B5CF6" }}>P {entry.result?.totals?.protein || 0}g</span>
                              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-lg" style={{ background: "rgba(245,158,11,0.12)", color: "#F59E0B" }}>C {entry.result?.totals?.carbs || 0}g</span>
                              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-lg" style={{ background: "rgba(16,185,129,0.12)", color: "#10B981" }}>F {entry.result?.totals?.fat || 0}g</span>
                            </div>
              ...
              {/* ── Column 3: Assessment ── */}
              <div style={{ background: "rgba(22,22,26,0.9)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, padding: 20 }} className="w-full xl:w-80 flex-shrink-0">
                {!selectedClientId ? (
                  <div className="flex flex-col items-center justify-center h-full py-12 opacity-40">
                    <ShieldAlert className="w-10 h-10 mb-3 text-white" />
                    <p className="text-white font-bold text-sm text-center">Select a client to view assessment</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-4 h-full">
                    <div className="flex items-center gap-2">
                      <Brain className="w-5 h-5" style={{ color: "#7CFC00" }} />
                      <h2 className="text-base font-black text-white">{selectedMember?.name}&apos;s Assessment</h2>
                    </div>
                    <button onClick={runAssessment} disabled={isAssessing}
                      className="w-full font-black text-sm px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                    ...
                    {assessment && (
                      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        style={{ background: `${assessment.threat_level_color}10`, borderColor: assessment.threat_level_color }}>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="font-black px-2.5 py-0.5 rounded-full text-[10px] uppercase" style={{ background: assessment.threat_level_color, color: "#000" }}>{assessment.threat_level}</span>
                          <span className="text-white font-medium text-xs">{assessment.summary}</span>
                        </div>
                        ...
                        <div className="text-xs font-bold text-white bg-black/30 p-2 rounded-xl border border-white/10">
                          💡 <span style={{ color: "#7CFC00" }}>{assessment.coach_recommendation}</span>
                        </div>
                      </motion.div>
                    )}
                    <div className="mt-auto grid grid-cols-2 gap-2">
                      {(['calories', 'protein', 'carbs', 'fat'] as const).map(k => (
                        <div key={k} className="flex items-center justify-between p-2 rounded-lg" style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.05)" }}>
                          <span className="text-[10px] font-bold uppercase" style={{ color: "#5A5A5A" }}>{k}</span>
                          <input type="number"
                            value={getClientGoals(selectedClientId)[k]}
                            onChange={e => saveClientGoal(selectedClientId, k, Number(e.target.value))}
                            className="w-12 bg-transparent font-bold text-right text-xs outline-none text-white" />
                        </div>
                      ))}
                    </div>
                  </div>
