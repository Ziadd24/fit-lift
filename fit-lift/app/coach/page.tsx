...
import { useListMembers, useListSessions, useCreateMember, useUpdateMember, useListCalorieLogs } from "@/lib/api-hooks";
...
export default function CoachDashboard() {
    ...
    const load = () => {
      try {
        ...
        if (goalsRaw) {
          ...
          // Standardize goals to handle objects or numbers back comp
          if (parsed && typeof parsed === 'object') {
             const refined: Record<string, number> = {};
             for (const k in parsed) {
                if (typeof parsed[k] === 'number') refined[k] = parsed[k];
                else if (parsed[k]?.calories) refined[k] = parsed[k].calories;
             }
             setClientGoals(refined);
          }
        }
      } catch {}
    };
  ...
  const { data: selectedClientLogs } = useListCalorieLogs(activeClientId);

  const clientDailyTotals = React.useMemo(() => {
    if (!selectedClientLogs) return { calories: 0, protein: 0, carbs: 0, fat: 0 };
    return selectedClientLogs.reduce((acc, l) => ({
      calories: acc.calories + (l.result?.totals?.calories || 0),
      protein: acc.protein + (l.result?.totals?.protein || 0),
      carbs: acc.carbs + (l.result?.totals?.carbs || 0),
      fat: acc.fat + (l.result?.totals?.fat || 0),
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-5 mb-8">
          ...
          <motion.div whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300 }}>
            <div style={{ ...cardStyle, padding: 24, height: 280, overflow: "hidden", position: "relative" }}>
              ...
              <div className="overflow-y-auto no-scrollbar" style={{ maxHeight: 200 }}>
                {activities.map((a) => (
                ...
                {isAddingExercise && (
                  <div className="mt-2 text-sm p-3 rounded-xl bg-white/5 border border-white/10 space-y-2">
                    <input type="text" placeholder="Workout Name (e.g. Pushups)" 
                      value={newExercise.name} onChange={e => setNewExercise({...newExercise, name: e.target.value})}
                      className="w-full bg-black/50 border border-white/10 rounded px-3 py-1.5 text-white outline-none" />
                    <div className="flex gap-2">
                       <input type="number" placeholder="Sets" 
                         value={newExercise.sets} onChange={e => setNewExercise({...newExercise, sets: e.target.value})}
                         className="w-1/2 bg-black/50 border border-white/10 rounded px-3 py-1.5 text-white outline-none" />
                    ...
                    <div className="flex gap-2 mt-2">
                      <button onClick={() => {
                        setActivities([...activities, { 
                          id: Date.now(), 
                          title: newExercise.name || "New Exercise", 
                          sub: `${newExercise.sets || 1} sets × ${newExercise.reps || 10} reps`, 
                          icon: Dumbbell, iconBg: "#7CFC00", pct: 0 
                        }]);
                        setIsAddingExercise(false);
                        setNewExercise({ name: "", sets: "", reps: "" });
                      }} className="flex-1 rounded bg-[#7CFC00] text-black font-bold py-1.5">Save</button>
                      <button onClick={() => {
                        setIsAddingExercise(false);
                        setNewExercise({ name: "", sets: "", reps: "" });
                      }} className="flex-1 rounded bg-red-500/20 text-red-500 font-bold py-1.5">Cancel</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          <motion.div whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300 }}>
            <div style={{ ...cardStyle, padding: 24, height: 280, display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              <div className="relative" style={{ marginTop: 8 }}>
                <SemiGauge percent={CALORIE_GOAL > 0 ? Math.min((clientDailyTotals.calories / CALORIE_GOAL) * 100, 100) : 0} />
              ...
              {isEditingGoal ? (
                ...
                <>
                  {clientDailyTotals.calories > 0 ? (
                    <div className="mt-2 text-center">
                      <p className="font-bold text-white relative group" style={{ fontSize: 22, fontFamily: "Inter,sans-serif" }}>
                        {clientDailyTotals.calories.toLocaleString()} <span className="text-sm font-normal" style={{ color: "#8B8B8B" }}>kcal</span>
                      </p>
                      <p className="text-xs mt-1 flex items-center justify-center gap-1" style={{ color: "#8B8B8B", fontFamily: "Inter,sans-serif" }}>
                        {Math.min(Math.round((clientDailyTotals.calories / CALORIE_GOAL) * 100), 100)}% of {CALORIE_GOAL.toLocaleString()} goal
                    ...
                    <div className="mt-2 text-center">
                      <p className="text-xs mt-1 flex items-center justify-center gap-1" style={{ color: "#5A5A5A", fontFamily: "Inter,sans-serif" }}>
                        No meals logged. Goal: {CALORIE_GOAL.toLocaleString()}
                        <button onClick={() => { setTempGoal(CALORIE_GOAL.toString()); setIsEditingGoal(true); }} className="hover:text-[#7CFC00]">
                          <Edit2 className="w-3 h-3" />
                        </button>
                      </p>
